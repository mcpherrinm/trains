...
def print_help():
    ...
    print "            (this means the meaning of pitch and clearance"
    print "            is inverted the sign of the spacing is changed)"
    print ""
    ...
    print ""
    print ""
    print "Argument types as used in abve options reference:"
...
try:
    opts, args = getopt.getopt(sys.argv[1:], "in:m:c:a:p:s:el:x:y:r:f:")
...
# phi_o = angle where the involute intersects the Do circle
phi_o = involute_intersect_with_r2(Dr/2, Do/2, 1.0, 1.0)[0]
...
for i in range(N):
    ...
    ax, ay = involute(Dr/2, phi2, phi2 + phi_o, 10)
    bx, by = involute(Dr/2, phi3, phi3 - phi_o, 10)
    if p != 0:
        aa = atan2(ay, ax)
        ar = sqrt(ay**2 + ax**2) + p
        ba = atan2(by, bx)
